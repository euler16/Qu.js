...
function isAntlrInvalidAltWarning(warning) {
	var message = typeof warning === 'string' ? warning : (warning && warning.message);
	return message && message.indexOf("Accessing non-existent property 'INVALID_ALT_NUMBER'") >= 0;
}

function loadAntlrRuntime() {
	if (typeof process === 'undefined' || !process.emitWarning) {
		return {
			antlr4: require('antlr4/index'),
			QASMLexer: require('./QASMLexer'),
			QASMParser: require('./QASMParser'),
			QASMListener: require('./QASMListener').QASMListener
		};
	}

	var emitWarning = process.emitWarning;
	process.emitWarning = function(warning) {
		if (isAntlrInvalidAltWarning(warning)) {
			return;
		}
...
var antlrRuntime = loadAntlrRuntime();
var antlr4 = antlrRuntime.antlr4;
var QASMLexer = antlrRuntime.QASMLexer;
var QASMParser = antlrRuntime.QASMParser;
var QASMListener = antlrRuntime.QASMListener;
...
var QASMImport = function(circuit, input, errorCallback) {
	...
	var totalQubits = QASM.numQbit();
	if(totalQubits > circuit.numQubits) {
		circuit.numQubits = totalQubits;
		while(circuit.gates.length < totalQubits) {
			circuit.gates.push([]);
		}
	}
	if(errorCallback) {
...
function normalizeGateName(gateName) {
	switch(gateName) {
		case "CX": return "cx";
		case "U": return "u3";
		default: return gateName;
	}
}

function qasmArgument(arg) {
	return {
		reg: arg.ID().getText(),
		bit: arg.INT() ? parseInt(arg.INT().getText()) : -1
	};
}

function expandCircuitArguments(listener, argContexts) {
	var args = argContexts.map(qasmArgument);
	var expandCount = 1;
	args.map(function(arg) {
		if(arg.bit < 0) {
			expandCount = Math.max(expandCount, listener.qregLen(arg.reg));
		}
	});

	var expanded = [];
	for(var x = 0; x < expandCount; x++) {
		var wires = [];
		args.map(function(arg) {
			var bit = arg.bit >= 0 ? arg.bit : x;
			wires.push(bit + listener.qregBase(arg.reg));
		});
		expanded.push(wires);
	}
	return expanded;
	...
	if(ctx.gatedecl()) {
		...
		if(ctx.goplist()) {
			...
			goplist.uop().map(function(uop) {
				var gateName = uop.ID() ? uop.ID().getText() : (uop.getChildCount() ? uop.getChild(0).getText() : "");
				if(gateName) {
					...
					var arglist = uop.argument ? uop.argument() : [];
					if(arglist && arglist.length) {
						var wires = [];
						arglist.map(function(arg) {
							wires.push(customGateRegs.indexOf(arg.ID().getText()));
						});
...
QCQASMListener.prototype.enterQop = function(ctx) {
	...
	var uop = ctx.uop();
	if(uop) {
		var gateName = normalizeGateName(uop.ID() ? uop.ID().getText() : (uop.getChildCount() ? uop.getChild(0).getText() : ""));
		if(!gateName) {
			return;
		}
		var idlist = uop.anylist() ? uop.anylist().idlist() : null;
		...
		var arglist = uop.argument ? uop.argument() : [];
		if(arglist && arglist.length) {
			expandCircuitArguments(self, arglist).map(function(wires) {
